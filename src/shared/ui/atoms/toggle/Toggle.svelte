...
<script lang="ts">
  ...
  const labelClass = $derived(`toggle-label toggle--${size} ${hasText ? 'toggle--with-text' : ''} ${className}`.trim());
...
<label class={labelClass}>
  <input {...restAttributes} type="checkbox" {checked} {disabled} class="toggle-input" onchange={handleChange} />
