<script lang="ts">
  ...
  const accordionLineCountSelected = $derived(toSelectedIndex(ACCORDION_LINE_COUNT_OPTIONS, accordionLineCount));
  ...
  const handleColorChange = (newIndex: number) => handleSelection(SANDBOX_COLOR_OPTIONS, newIndex, onChangeColor);
  ...
  const handleTableGroupActionsChange = (value: boolean) => handleToggleChange(value, onChangeTableGroupActions);

  const handleTableRowDragChange = (value: boolean) => handleToggleChange(value, onChangeTableRowDrag);
