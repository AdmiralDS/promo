  ...
  return (
    <Wrapper>
      <ContentSwitcher dimension={'l'} appearance="primary">
        {list.map((item, index) => {
          return (
            <ContentSwitcherItem key={index} active={index === active1} onClick={() => setActive1(index)}>
              {item.title}
            </ContentSwitcherItem>
          );
        })}
      ...
      <ContentSwitcher dimension={'l'}>
        {list.map((item, index) => {
          return (
            <ContentSwitcherItem key={index} active={index === active2} onClick={() => setActive2(index)}>
              {item.title}
            </ContentSwitcherItem>
          );
        })}
